...
Each chip is represented by a table.
The columns corresponds to the operands relevant to the given operation, as well as auxiliarly ``helper columns''.
The rows correspond to the different executions of the instruction in the given shard.
The chip is further associated with algebraic
constraints (AIRs) between its columns, which are designed to check all or part of the operation'scorrectness (via the zerocheck protocol, described below).
...
global multiset equality statement.
...
to delegate some of the precompile correctness
...
\item \texttt{compose}:
An $N$-to-1 recursion circuit ($2\leq N\leq 4$) \rnote{maybe $2 \leq N \leq 4$?} that verifies multiple
