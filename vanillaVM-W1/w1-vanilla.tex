...
It is typically not possible to prove an entire trace $\TTT$ at once on a single machine; it is therefore necessary to
split the execution into shards.
We now explain how SP1 undertakes this splitting.
...
\item \textbf{Commit.}
      ...
      and commit to it via Reed--Solomon encoding and Merkle hashing
...
\begin{enumerate}
\item Interpret each chunk as the evaluations of a univariate polynomial of
...
\item Hash the encoded columns into a single Merkle tree;
      the Merkle root constitutes the trace commitment.
\end{enumerate}
...
depth $n_{row}$; SP1 proves the correctness of this circuit evaluation via the GKR polynomial IOP.
...
We use the jagged assist (as described in the reference) to further reduce verifier work in the evaluation
